...
### Multimodal AI OCR Fallback Architecture

Standard PDF parsing libraries (like `pdfjs-dist` or `WebPDFLoader`) frequently fail on complex documents, scanned images, and non-English languages (like Hindi or Arabic). To solve this, we implemented a robust **Multimodal AI OCR Fallback**:

1. **Primary Parser (Fast & Free):** The system first attempts to extract text using the standard local Node.js PDF loader.
2. **Failure Detection:** If the extracted text is suspiciously short (e.g., `< 50` characters), the system flags the document as a scanned image or unsupported font.
3. **AI Vision Extraction:** The entire PDF binary is base64-encoded and sent to Google's `gemini-1.5-flash` Multimodal Vision Engine. We bypass traditional OCR tools like Tesseract.
4. **Regex Prompt Engineering:** We use a highly specific prompt instructing the AI to wrap every page in `[PAGE_X_START]` and `[PAGE_X_END]` tags.
5. **Reconstruction:** A custom Regex parser rebuilds the pseudo-LangChain `Document` chunks with exact `pageNumber` metadata so the citation engine remains intact even on AI-extracted text.
...
**Q12: You mentioned storing 50 chunks into a vector database. Did you ever encounter silent failures when calling external APIs?**
*Answer:* Yes, a major architectural hurdle was the Embeddings API. When we attempted to batch-process 50 chunks (about 50,000 characters) into the Gemini Embeddings API in a single network request, the API would occasionally choke on the payload size or trigger a safety filter. Because LangChain's wrapper caught the error internally, it silently returned an empty array of vectors, meaning our database transaction succeeded but inserted 0 rows! To architect around this, I rewrote the insertion engine to embed chunks sequentially. This isolates failures to individual chunks and guarantees data integrity at the cost of a few extra seconds of processing time.
...
## 9. AWS EC2 Deployment Guide (t3.micro)

Deploying a full-stack Next.js application with a Redis container and a separate background worker on a 1GB RAM `t3.micro` instance requires careful memory management.

### Why this approach?

* **Swap Space:** A `t3.micro` only has 1GB of RAM. Running `npm run build` or Next.js compilation will inevitably cause an Out-Of-Memory (OOM) crash. We mitigate this by creating a 2GB Swap file (using the SSD storage as overflow RAM).
* **PM2 for Node Processes:** While we use Docker for Redis, wrapping the Next.js Web Server and the BullMQ Worker in Docker containers adds virtualization overhead. Using PM2 (Process Manager 2) allows us to run the Next.js server and the background worker natively on the host machine, using fewer resources and automatically restarting them if they crash.
* **Docker for Redis:** Redis is extremely lightweight. Using Docker for Redis avoids the hassle of dealing with Ubuntu package managers and isolates the queue.

### Step 0: Provisioning the AWS EC2 Instance (AWS Console)

Before running any code, you need to spin up the actual server in AWS:

1. Log into your **AWS Console** and go to the **EC2 Dashboard**.
2. Click **Launch Instance**.
3. **Name:** Enter `NoteSage-Server`.
4. **OS Image (AMI):** Select **Ubuntu**, and choose `Ubuntu Server 24.04 LTS (HVM)` (or the latest LTS version).
5. **Instance Type:** Select **`t3.micro`** (or `t2.micro` if you are on the older free tier).
6. **Key Pair:** Click "Create new key pair", name it `notesage-key`, select **RSA** and **.pem**, and download it. Keep this file safe!
7. **Network Settings:**
   * Check **Allow SSH traffic from Anywhere**.
   * Check **Allow HTTP traffic from the internet**.
   * Check **Allow HTTPS traffic from the internet**.
8. **Storage:** Increase the Root volume to at least **15 GB** `gp3` (the free tier allows up to 30GB). You will need this space for Docker and Next.js builds.
9. Click **Launch Instance**.

### Step 0.5: Connecting to your Server (SSH)

Once the instance says "Running" in the AWS console, copy its **Public IPv4 address**.
Open your terminal on your computer (wherever your `.pem` key was downloaded) and run:

```bash
# (Mac/Linux only) Restrict permissions on your key so AWS accepts it
chmod 400 notesage-key.pem

# SSH into the server using the Ubuntu default user
ssh -i "notesage-key.pem" ubuntu@<YOUR_EC2_PUBLIC_IP>
```

### Step-by-Step Command Guide (Run these ON the EC2 Server)
...
**2. Create a Swap File (CRITICAL FOR t3.micro)**

```bash
# Create a 2GB swap file to prevent Out of Memory crashes during build
...
nano .env 
# (Paste all your credentials: DATABASE_URL, AWS keys, Gemini, etc. Save and exit)
```

**4. Start Redis using Docker**

```bash
# This will spin up the redis:alpine container in the background
sudo docker-compose up -d redis
```

**5. Build the Application & Generate Prisma**

```bash
# Generate the Prisma client for database access
npx prisma generate
# Push the schema if needed (only on first deploy, otherwise run migrate deploy)
npx prisma db push
# Build the optimized production bundle for Next.js
npm run build
```
...
pm2 start npm --name "notesage-web" -- run start

# Start the BullMQ Background Worker
pm2 start npm --name "notesage-worker" -- run worker

# Save the PM2 list so they restart automatically if the server reboots
pm2 save
pm2 startup
```

**7. Setup Reverse Proxy (Nginx - Optional but Recommended)**
Since Next.js runs on port 3000, you use Nginx

to map port 80 (HTTP) to port 3000.

```bash
sudo apt install nginx -y
# Configure /etc/nginx/sites-available/default to proxy_pass to http://localhost:3000
